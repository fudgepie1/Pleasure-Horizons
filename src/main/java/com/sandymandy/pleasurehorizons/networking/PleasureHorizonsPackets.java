...
public class PleasureHorizonsPackets {
    ...
    public static void registerPackets(){
        PleasureHorizons.LOGGER.info("Registering Packet Codecs for " + PleasureHorizons.MOD_NAME);
        // --- C2S (client → server) ---
    ...
    public static void registerC2SPackets(){
        PleasureHorizons.LOGGER.info("Registering C2S Packets for " + PleasureHorizons.MOD_NAME);
        // --- C2S (client → server) ---
                ...
                (packet, context) -> Objects.requireNonNull(context.player().getServer()).execute(() -> {
                            ...
                            if (entity instanceof TameableGirlEntity girl) {
                                switch (packet.actionId()) {
                                    ...
                                    default -> PleasureHorizons.LOGGER.warn("Unknown Girl interaction: " + packet.actionId());
                                }
                            }
                        }
                ));
